...
Google.init = async function ({ router, middleware }) {
	...
	const settings = await meta.settings.get(Google.meta.settingsKey);
	...
	Google.settings.autoconfirm = settings.autoconfirm === 'on';
	Google.settings.style = settings.style || 'light';
	Google.settings.disableRegistration = settings.disableRegistration === 'on';
};
...
Google.addMenuItem = function (header) {
	header.authentication.push({
		route: `/plugins/sso-${Google.meta.id}`,
		icon: Google.meta.icon,
		name: Google.meta.name,
	});
	return header;
};
