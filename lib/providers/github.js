...
Github.init = async function ({ router, middleware }) {
	...
	hostHelpers.setupAdminPageRoute(router, `/admin/plugins/sso-${id}`, (req, res) => {
	...
	const settings = await meta.settings.get(Github.meta.settingsKey);
	...
	Github.settings.needToVerifyEmail = settings.needToVerifyEmail === 'on';
	Github.settings.disableRegistration = settings.disableRegistration === 'on';
};
...
Github.login = async function (req, githubID, displayName, username, email, pictureUrl) {
	...
	if (Github.settings.disableRegistration) {
	...
	return await User.createOrQueue(req, {
		...
		emailVerification: Github.settings.needToVerifyEmail ? 'send' : 'verify',
	});
};
...
Github.addMenuItem = function (header) {
	header.authentication.push({
		route: `/plugins/sso-${Github.meta.id}`,
		icon: Github.meta.icon,
		name: Github.meta.name,
	});
	return header;
};
